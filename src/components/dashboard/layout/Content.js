...
export default function ContentDashBoard() {
  ...
  const { data: comment } = useSWR("/comment", fetcher);
  const { data: article } = useSWR("/post", fetcher);
  const { data: tag } = useSWR("/hash", fetcher);
  const { data: report } = useSWR("/report", fetcher);
